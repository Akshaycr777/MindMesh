...
from database import (
    init_db,
    create_or_update_the_conversation,
    list_conversations,
    save_chat_message,
    chat_history
)
...
async def upload_document(
    ...
    try:
        allowed_extentions = [".pdf",".docx",".txt",".md",".csv",".log",".json"]

        filename = file.filename or "uploaded_file"
        suffix = Path(filename).suffix.lower()

        if suffix not in allowed_extentions:
            return JSONResponse(
                {
                    "success": False,
        ...
        create_or_update_the_conversation(thread_id, "Uploaded Document")
...
async def chat_stream(request: Request):
    ...
    selected_model = data.get("model", "gemini-3.1-flash-lite")
    ...
    create_or_update_the_conversation(thread_id, user_message)
